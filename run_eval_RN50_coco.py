...
for name, path in paths.items():
    ...
    checkpoint = os.path.join(
            path, f"global_step{15000}/mp_rank_00_model_states.pt"
        )
    for temp in [0.01]:
        config_path = configs[name]
        # cmd = f"deepspeed --include localhost:0  --master_port 6912 eval.py {config_path} --task_induction \"A picture of\" --checkpoint {checkpoint} --data_dir /mnt/localdisk/ --tasks nocaps --few_shot_examples  0 1 2 4 --save_path eval_results/{name}_temp_{temp}_{step}_nocaps_task_induction_step.json"
        # try:
        #     os.system(cmd)
        # except:
        ...
        #         f.write(exc)
        cmd = f'deepspeed --include localhost:0 --master_port 6867 eval.py {config_path} --task_induction "A picture of" --checkpoint {checkpoint} --data_dir /mnt/localdisk/ --tasks coco --few_shot_examples  0 --save_path eval_results/{name}_temp_{temp}_{step}_coco_task_induction_rerun.json'
        try:
            os.system(cmd)
        except:
            print(f"Failed to run {name}")
